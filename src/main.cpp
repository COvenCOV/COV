...
unsigned int nModifierInterval = 10 * 60; // Coven - time to elapse before new modifier is computed
...
const string strMessageMagic = "Coven Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Coven", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "16 Apr 2015 - Bitcoin is the worlds most dangerous idea.";
        CTransaction txNew;
        txNew.nTime = 1429218900;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1429218900;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 48433311;
        uint256 hash = block.GetHash();
        printf("%s\n", hash.ToString().c_str());
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x8586d2c63b31febb3e5da94667d27c507a8f2281eb24885dad864786937514c2"));
